...
const fight = (...fighters) => {
  toggleGameField();
  fighters.forEach(fighter => assingProp(fighter, "healthCulc", culcHealthRest(fighter.health)));
...
const fillFightersInfo = fighters => {
  [firstFighterTarget, secondFighterTarget].forEach((target, i) => updateFighterTarget(fighters[i], target));
  firstFighterHit ? restartAnimation(firstFighterTarget) : restartAnimation(secondFighterTarget);
};
...
const culcHealthRest = initial => current => Math.floor(current * 100 / initial);
...
class FighterView extends _view__WEBPACK_IMPORTED_MODULE_0__["default"] {
  ...
  createFighter(fighter, handleDetalisClick, handleSelectClick) {
    ...
    selectBtn.addEventListener("click", event => handleSelectClick(event, fighter), false);
    fighterMain.append(imageElement, nameElement, selectBtn);
    this.element.append(fighterMain, selectBtn);
    fighterMain.addEventListener("click", event => handleDetalisClick(event, fighter), false);
  }
...
class FightersView extends _view__WEBPACK_IMPORTED_MODULE_0__["default"] {
  constructor(fighters) {
    ...
    _defineProperty(this, "modal", document.querySelector(".modal-info"));

    _defineProperty(this, "modalTitle", document.querySelector(".modal-title"));

    _defineProperty(this, "healthInfo", document.getElementById("health-info"));

    _defineProperty(this, "attackInfo", document.getElementById("attack-info"));

    _defineProperty(this, "defenseInfo", document.getElementById("defense-info"));

    _defineProperty(this, "startFightBtn", document.getElementById("start-fight"));

    _defineProperty(this, "fighters", []);

    _defineProperty(this, "selectedColor", "blue");
    ...
    this.startFightBtn.addEventListener("click", () => this.startFightClick(this.fighters));
  }

  startFightClick(fighters) {
    fighters.length < 2 ? this.startFightBtn.nextElementSibling.classList.remove("visually-hidden") : this.startFight(fighters);
  }
  ...
  async startFight([{
    ...
    Object(_fight__WEBPACK_IMPORTED_MODULE_3__["fight"])(new _Fighter__WEBPACK_IMPORTED_MODULE_4__["default"](firstFighter), new _Fighter__WEBPACK_IMPORTED_MODULE_4__["default"](secondFighter));
    this.startFightBtn.nextElementSibling.classList.add("visually-hidden");
  }

  createFighters(fighters) {
    const fighterElements = fighters.map(fighter => {
      const fighterView = new _fighterView__WEBPACK_IMPORTED_MODULE_1__["default"](fighter, this.handleDetailsClick, this.handleSelectClick);
  ...
  async handleFighterSelectClick(event, {
    _id: fighterID
  }) {
    await this.getFighterDetails(fighterID);

    if (this.fighters.length === 2) {
      this.fighters.shift().target.classList.remove("selected-fighter", "red", "blue");
    }
  ...
  async handleFighterClick(event, {
    _id: fighterID
  }) {
  ...
  async getFighterDetails(id) {
    ...
    if (!fighterDetails) {
      fighterDetails = await _services_fightersService__WEBPACK_IMPORTED_MODULE_2__["fighterService"].getFighterDetails(id);
  ...
  async updateFighterDetails(fighter) {
    ...
    const res = await _services_fightersService__WEBPACK_IMPORTED_MODULE_2__["fighterService"].updateFighterInfo(updatedFighter);
    this.fightersDetailsMap.set(res._id, res);
  }

  async removeFighter(fighter) {
    const res = await _services_fightersService__WEBPACK_IMPORTED_MODULE_2__["fighterService"].deleteFighter(fighter._id);
  ...
  initModal(fighter) {
    ...
    newDeleteBtn.addEventListener("click", async () => {
      ...
      if (res) {
        ...
        const fighters = await _services_fightersService__WEBPACK_IMPORTED_MODULE_2__["fighterService"].getFighters();
        this.createFighters(fighters);
    ...
    newSaveBtn.addEventListener("click", () => (this.updateFighterDetails(fighter), this.closeModal()));
  }
...
_defineProperty(FightersView, "rootElement", document.getElementById("root"));
...
function callApi(endpoind, method, data) {
  ...
  if (method === "PUT") {
    ...
    }).then(response => response.ok ? response.json() : Promise.reject(Error("Failed to load"))).catch(error => {
      throw error;
    });
  }
...
class FighterService {
  ...
  async updateFighterInfo(data) {
    try {
      ...
      const apiResult = await Object(_helpers_apiHelper__WEBPACK_IMPORTED_MODULE_0__["callApi"])(endpoint, "PUT", data);
      return apiResult;
    } catch (error) {
      throw error;
    }
  ...
  async deleteFighter(_id) {
    try {
      const endpoint = `user/${_id}`;
      const apiResult = await Object(_helpers_apiHelper__WEBPACK_IMPORTED_MODULE_0__["callApi"])(endpoint, "DELETE");
      return apiResult;
    } catch (error) {
      throw error;
    }
